...
create table if not exists cuentas(id int primary key auto_increment,diario_id int,nombre varchar(155),constraint fk_cuenta_diario foreign key (diario_id) references diarios(id) on update cascade on delete cascade);
create table if not exists anotaciones(id int primary key auto_increment,cuenta_id int,asiento_id int,debe int,haber int,constraint fk_anotacion_cuenta foreign key (cuenta_id) references cuentas(id) on update cascade on delete cascade,constraint fk_anotacion_asiento foreign key (asiento_id) references asientos(id) on update cascade on delete cascade);
